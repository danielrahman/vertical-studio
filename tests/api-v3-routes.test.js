...
test('bootstrap-from-extraction normalizes low-confidence fields into TODO entries', async () => {
  ...
  try {
    ...
    const nonObjectSitePolicyResponse = await fetch(
      `${baseUrl}/api/v1/sites/site-bootstrap-model/bootstrap-from-extraction`,
      {
        method: 'POST',
        headers: INTERNAL_ADMIN_HEADERS,
    ...
    assert.equal(nonObjectSitePolicyResponse.status, 400);
    const nonObjectSitePolicyPayload = await nonObjectSitePolicyResponse.json();
    assert.equal(nonObjectSitePolicyPayload.code, 'validation_error');
...
test('cms webhook publish ingress rejects unknown top-level payload fields', async () => {
  ...
  try {
    ...
    assert.deepEqual(body.details.receivedAllowedTopLevelFields, ['event', 'siteId']);
    assert.deepEqual(body.details.receivedAllowedTopLevelFieldIndexes, [0, 1]);
...
test('copy generate rejects unsupported highImpactOnlyThreeVariants mode when provided and not true', async () => {
  ...
  try {
    ...
    assert.equal(invalidFlagBody.details.expectedType, 'boolean');
    assert.equal(invalidFlagBody.details.receivedType, 'boolean');
...
test('copy generate enforces locale allow-list and rejects duplicate locales', async () => {
  ...
  try {
    const nonArrayLocalesRes = await fetch(`${baseUrl}/api/v1/sites/site-copy-locales/copy/generate`, {
      method: 'POST',
      headers: INTERNAL_ADMIN_HEADERS,
      body: JSON.stringify({
    ...
    const nonArrayLocalesBody = await nonArrayLocalesRes.json();
    ...
    assert.deepEqual(invalidLocaleBody.details.unsupportedLocales, ['de-DE', 'zz-ZZ']);
    assert.deepEqual(invalidLocaleBody.details.allowedLocales, ['cs-CZ', 'en-US']);
    ...
    assert.deepEqual(duplicateLocaleBody.details.duplicateLocales, ['cs-CZ', 'en-US']);
  } finally {
    await stopServer(server);
  }
});
...
test('copy select per-item validation failures report invalidField and selection-index details', async () => {
  const { server, baseUrl } = await startServer();
...
test('overrides rejects unknown top-level payload fields', async () => {
  ...
  try {
    ...
    assert.equal(invalidOverrideBody.details.invalidField, 'payload');
    assert.deepEqual(invalidOverrideBody.details.unknownFields, ['alphaUnsupportedDirective', 'zetaUnsupportedDirective']);
    assert.deepEqual(invalidOverrideBody.details.allowedTopLevelFields, [
      'actorRole',
      'draftId',
      'excludedCompetitorPatterns',
      'excludedSections',
...
test('public runtime compatibility snapshot fallback prefers valid generatedAt entries over invalid or missing duplicate mappings', async () => {
  const { app, server, baseUrl } = await startServer();

  try {
    const siteId = 'site-runtime-compat-valid-generatedat-preferred';
    const publishRes = await fetch(`${baseUrl}/api/v1/sites/${siteId}/publish`, {
      method: 'POST',
      headers: INTERNAL_ADMIN_HEADERS,
      body: JSON.stringify({
        draftId: 'draft-runtime-compat-valid-generatedat-preferred',
        proposalId: 'proposal-runtime-compat-valid-generatedat-preferred',
        host: 'compat-valid-generatedat-preferred.example.test'
      })
    });
    assert.equal(publishRes.status, 200);
    const publishBody = await publishRes.json();
    ...
    state.runtimeSnapshotsByStorageKey.set(historicalStorageKeyInvalid, {
      ...publishedSnapshot,
      proposalId: 'proposal-runtime-compat-historical-invalid',
      generatedAt: 'not-a-date'
    });
    state.runtimeSnapshotsByStorageKey.set(historicalStorageKeyMissing, {
      ...publishedSnapshot,
      proposalId: 'proposal-runtime-compat-historical-missing',
      generatedAt: null
    });
    state.runtimeSnapshotsByStorageKey.set(historicalStorageKeyValid, {
      ...publishedSnapshot,
      proposalId: 'proposal-runtime-compat-historical-valid',
    ...
    assert.equal(compatibilitySnapshotBody.snapshot.proposalId, 'proposal-runtime-compat-historical-valid');
...
test('published runtime snapshot prefers selected copy recommendations for mapped runtime slots', async () => {
  ...
  try {
    ...
    const selectRes = await fetch(`${baseUrl}/api/v1/sites/${siteId}/copy/select`, {
      ...
      body: JSON.stringify({
        ...
        selections: [
          ...
          {
            slotId: 'process.step_1_title',
            locale: 'cs-CZ',
            candidateId: stableId(`${draftId}|process.step_1_title|cs-CZ|SINGLE`)
          },
          {
            slotId: 'faq.q1',
            locale: 'cs-CZ',
            candidateId: stableId(`${draftId}|faq.q1|cs-CZ|SINGLE`)
          },
          {
            slotId: 'faq.a1',
            locale: 'cs-CZ',
            candidateId: stableId(`${draftId}|faq.a1|cs-CZ|SINGLE`)
          },
          {
    ...
    const processSection = snapshotBody.snapshot.sections.find((section) => section.sectionId === 'process');
    assert.ok(processSection);
    assert.equal(processSection.slots.step1Title, 'process_step_1_title_cs_SINGLE');

    const faqSection = snapshotBody.snapshot.sections.find((section) => section.sectionId === 'faq');
    assert.ok(faqSection);
    assert.equal(faqSection.slots.question1, 'faq_q1_cs_SINGLE');
    assert.equal(faqSection.slots.answer1, 'faq_a1_cs_SINGLE');
...
test('secret refs endpoint enforces internal_admin ACL, naming policy, and metadata-only payloads', async () => {
  ...
  try {
    ...
    const invalidProviderShapeBody = await invalidProviderShapeRes.json();
    assert.equal(invalidProviderShapeBody.code, 'validation_error');
    assert.equal(invalidProviderShapeBody.details.invalidField, 'provider');
    assert.equal(invalidProviderShapeBody.details.expectedPattern, '^[a-z0-9-]+$');
    assert.equal(invalidProviderShapeBody.details.receivedValue, 'OpenAI');
    ...
    const invalidKeyShapeBody = await invalidKeyShapeRes.json();
    assert.equal(invalidKeyShapeBody.code, 'validation_error');
    assert.equal(invalidKeyShapeBody.details.invalidField, 'key');
    assert.equal(invalidKeyShapeBody.details.expectedPattern, '^[a-z0-9-]+$');
    assert.equal(invalidKeyShapeBody.details.receivedValue, 'api!');
    ...
    const invalidTenantSlugShapeBody = await invalidTenantSlugShapeRes.json();
    assert.equal(invalidTenantSlugShapeBody.code, 'validation_error');
    assert.equal(invalidTenantSlugShapeBody.details.invalidField, 'tenantSlug');
    assert.equal(invalidTenantSlugShapeBody.details.expectedPattern, '^[a-z0-9-]+$');
    assert.equal(invalidTenantSlugShapeBody.details.receivedValue, 'tenant_1');
