...
test('WS-E contract: published runtime snapshot applies selected copy recommendations for mapped slots', async () => {
  ...
  try {
    ...
    const heroPrimaryCtaCandidate = candidates.find(
      (candidate) =>
    ...
    const aboutIntroCandidate = candidates.find(
      (candidate) => candidate.slotId === 'about.intro' && candidate.locale === 'cs-CZ' && candidate.variantKey === 'B'
    );
    assert.ok(aboutIntroCandidate);
    const processStep1TitleCandidate = candidates.find(
      (candidate) =>
        candidate.slotId === 'process.step_1_title' &&
        candidate.locale === 'cs-CZ' &&
        candidate.variantKey === 'SINGLE'
    );
    assert.ok(processStep1TitleCandidate);
    const faqQuestion1Candidate = candidates.find(
      ...
      body: JSON.stringify({
        ...
        selections: [
          ...
          },
          {
            slotId: 'about.intro',
            locale: 'cs-CZ',
            candidateId: aboutIntroCandidate.candidateId
          },
          {
            slotId: 'process.step_1_title',
            locale: 'cs-CZ',
            candidateId: processStep1TitleCandidate.candidateId
          },
          {
            slotId: 'faq.q1',
            locale: 'cs-CZ',
    ...
    assert.equal(aboutSection.slots.intro, aboutIntroCandidate.text);

    const processSection = snapshotBody.snapshot.sections.find((section) => section.sectionId === 'process');
    assert.ok(processSection);
    assert.equal(processSection.slots.step1Title, processStep1TitleCandidate.text);
...
test('WS-E contract: runtime resolve preserves compatibility fallback identifiers when storageKey is unavailable', async () => {
  const { app, server, baseUrl } = await startServer();

  try {
    const siteId = 'site-wse-runtime-fallback-shape';
    const publishRes = await fetch(`${baseUrl}/api/v1/sites/${siteId}/publish`, {
      method: 'POST',
      headers: INTERNAL_ADMIN_HEADERS,
      body: JSON.stringify({
        draftId: 'draft-wse-runtime-fallback-shape',
        proposalId: 'proposal-wse-runtime-fallback-shape',
        host: 'wse-fallback-shape.example.test'
      })
    });
    assert.equal(publishRes.status, 200);
    const publishBody = await publishRes.json();
    ...
    const resolveRes = await fetch(
      `${baseUrl}/api/v1/public/runtime/resolve?host=${encodeURIComponent('  HTTPS://WSE-Fallback-Shape.Example.Test:443/path  ')}`
    );
    assert.equal(resolveRes.status, 200);
    const resolveBody = await resolveRes.json();

    assert.equal(resolveBody.host, 'wse-fallback-shape.example.test');
...
test('WS-E contract: compatibility snapshot fallback deterministically selects latest snapshot when duplicate site/version mappings exist', async () => {
  const { app, server, baseUrl } = await startServer();

  try {
    const siteId = 'site-wse-compat-deterministic-duplicates';
    const publishRes = await fetch(`${baseUrl}/api/v1/sites/${siteId}/publish`, {
      method: 'POST',
      headers: INTERNAL_ADMIN_HEADERS,
      body: JSON.stringify({
        draftId: 'draft-wse-compat-deterministic-duplicates',
        proposalId: 'proposal-wse-compat-deterministic-duplicates',
        host: 'wse-compat-deterministic-duplicates.example.test'
      })
    });
    assert.equal(publishRes.status, 200);
    const publishBody = await publishRes.json();
