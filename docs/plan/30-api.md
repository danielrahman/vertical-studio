...
8. When compatibility lookup finds multiple immutable snapshots for the same `siteId+versionId`, selection is deterministic: newest valid `snapshot.generatedAt` wins, and ties are resolved by lexicographically smallest `storageKey`.
