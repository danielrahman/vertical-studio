...
11. Selection-level validation failures for `selections` (empty array, tuple mismatch, duplicate tuple, selectedBy actor mismatch) return deterministic `invalidField` details. Index metadata is required where a concrete failing tuple exists: missing-candidate errors include `selectionIndex` (Rule 4), tuple-mismatch errors include `selectionIndex` with candidate-vs-request comparison details (`candidateSlotId`, `candidateLocale`, `requestedSlotId`, `requestedLocale`), duplicate-tuple errors include `firstSelectionIndex` and `duplicateSelectionIndex`, and selectedBy actor-mismatch errors include `selectionIndex`.
12. Per-item `selections` validation failures (item type, unknown fields, missing/invalid `slotId|locale|candidateId`, invalid `selectedBy`) return deterministic `invalidField` details using index-aware paths (for example `selections[0].locale`); item-type failures, unknown per-item field failures, missing/invalid `slotId|locale|candidateId` failures, and invalid `selectedBy` allow-list failures include deterministic `selectionIndex`; unknown per-item fields include deterministic `allowedSelectionFields` details with lexicographically sorted values, invalid locale values include deterministic `allowedLocales` details with lexicographically sorted values, and invalid `selectedBy` values include deterministic `allowedSelectedByRoles` details with lexicographically sorted values.
