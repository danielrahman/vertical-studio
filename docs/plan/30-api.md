...
11. Selection-level validation failures for `selections` (empty array, tuple mismatch, duplicate tuple, selectedBy actor mismatch) return deterministic `invalidField` details; tuple-mismatch errors include deterministic `selectionIndex` and candidate-vs-request comparison details (`candidateSlotId`, `candidateLocale`, `requestedSlotId`, `requestedLocale`); duplicate-tuple errors include deterministic `firstSelectionIndex` and `duplicateSelectionIndex` details; selectedBy actor-mismatch errors include deterministic `selectionIndex`.
