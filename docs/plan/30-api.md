...
6. Reassigning an existing secret ref to a different `tenantId` is rejected with `409 secret_ref_conflict` and deterministic `invalidField: tenantId` metadata.
