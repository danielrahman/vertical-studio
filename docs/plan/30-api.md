...
3. Unknown top-level payload fields are rejected with `400 validation_error`, deterministic `invalidField: payload`, lexicographically sorted `unknownFields` details, deterministic `receivedTopLevelFieldCount`, lexicographically sorted `receivedTopLevelFields` metadata, and lexicographically sorted `allowedTopLevelFields` metadata.
